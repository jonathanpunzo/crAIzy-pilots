# 🚀 IBM SkillsBuild Project

Repository ufficiale del progetto sviluppato nell’ambito del percorso formativo **IBM SkillsBuild**.

Il progetto nasce come attività di gruppo con l’obiettivo di applicare in modo pratico le competenze acquisite durante i corsi, lavorando su un’idea concreta, documentata e sviluppata in maniera collaborativa.

---

## 👥 Team

**Nome gruppo:** SkillBuilders Team

| Nome | Ruolo |
|---|---|
| Jonathan Punzo | Membro del team |
| Felice Iandoli | Membro del team |
| Andrea Botta | Membro del team |
| Mariagiusy Cientanni | Membro del team |
| Simona Ravotti | Membro del team |

---

## 📌 Descrizione del progetto

Questo repository contiene il materiale, il codice e la documentazione prodotti durante il percorso IBM SkillsBuild.

Al momento il progetto è in fase iniziale: la struttura verrà aggiornata progressivamente con l’avanzamento delle attività, l’aggiunta dei contenuti tecnici e lo sviluppo delle funzionalità previste.

---

## 🎯 Obiettivi

Gli obiettivi principali del progetto sono:

- applicare le competenze acquisite nei corsi IBM SkillsBuild;
- lavorare in team su un progetto condiviso;
- organizzare correttamente codice, documentazione e materiali;
- sviluppare una soluzione chiara, presentabile e facilmente comprensibile;
- documentare ogni fase del lavoro in modo ordinato e professionale.

---

## 🧠 Competenze coinvolte

Durante il progetto verranno approfonditi temi legati a:

- problem solving;
- collaborazione tramite repository GitHub;
- documentazione tecnica;
- sviluppo software;
- intelligenza artificiale e tecnologie digitali;
- analisi, progettazione e presentazione di una soluzione.

---

## 📂 Struttura del repository

```text
IBM-SkillsBuild-Project/
│
├── README.md
├── docs/
│   └── documentazione del progetto
│
├── src/
│   └── codice sorgente
│
├── assets/
│   └── immagini, grafici e materiali utili
│
└── presentation/
    └── slide e materiali per la presentazione finale
