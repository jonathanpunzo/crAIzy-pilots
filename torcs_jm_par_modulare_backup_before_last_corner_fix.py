...
DRIVER_VERSION = 'stable_steer_v2_clean'
...
def limit_change(current_value, target_value, max_change):
    change = clip(target_value - current_value, -max_change, max_change)
...
def calculate_steering(S, R, track_info):
    angle = S['angle']
    track_pos = S['trackPos']
    previous_steer = get_value(R, 'steer')
    ...
    if track_info['is_straight']:
        ...
        angle_dead_zone = 0.008
        position_dead_zone = 0.02
    ...
    target_steer = (angle * angle_gain / PI) - (track_pos * centering_gain) + sensor_steer

    if abs(track_pos) > 0.85:
        target_steer -= track_pos * 0.25
    ...
    curve_direction = sign(track_info['curve_signal'])
    if curve_direction == 0:
        curve_direction = sign(angle)

    if not track_info['is_straight'] and curve_direction != 0 and abs(target_steer) < CURVE_STEER_HOLD:
        target_steer = curve_direction * CURVE_STEER_HOLD

    if abs(track_pos) > 0.85:
        max_change = 0.18
    elif S['speedX'] > 170:
        max_change = 0.07 if track_info['is_straight'] else 0.10
    elif S['speedX'] > 120:
        max_change = 0.09 if track_info['is_straight'] else 0.13
    else:
        max_change = 0.14 if track_info['is_straight'] else 0.16

    steer = limit_change(previous_steer, target_steer, max_change)
...
def drive_modular(c):
    ...
    R['steer'] = calculate_steering(S, R, track_info)
