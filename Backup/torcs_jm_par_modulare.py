...
DRIVER_VERSION = 'stable_last_corner_v1'
...
SLIP_SOFT_LIMIT = 2
SLIP_HARD_LIMIT = 6
FINAL_CORNER_START = 3260
FINAL_CORNER_MID = 3420
FINAL_CORNER_EXIT = 3540
FINAL_CORNER_ENTRY_SPEED = 115
FINAL_CORNER_APEX_SPEED = 88
FINAL_CORNER_EXIT_SPEED = 125
LOG_DIRECTORY = 'logs'
...
LOG_FIELDS = [
    ...
    'max_abs_steer',
    'avg_abs_steer',
    'avg_corner_abs_steer',
    'avg_straight_abs_steer',
    'steer_zero_steps',
    'corner_steer_zero_steps',
    'steer_direction_changes',
    'corner_steer_direction_changes',
    'max_accel',
    'max_brake',
    'brake_steps',
    'throttle_steps',
...
def is_final_corner(S):
    dist_from_start = get_value(S, 'distFromStart')
    return FINAL_CORNER_START <= dist_from_start <= 3615

def final_corner_target_speed(S):
    dist_from_start = get_value(S, 'distFromStart')

    if dist_from_start < FINAL_CORNER_MID:
        return FINAL_CORNER_ENTRY_SPEED
    if dist_from_start < FINAL_CORNER_EXIT:
        return FINAL_CORNER_APEX_SPEED
    return FINAL_CORNER_EXIT_SPEED
...
class DriveLogger():
    ...
    def reset_stats(self):
        ...
        self.corner_abs_steer_sum = 0.0
        self.straight_abs_steer_sum = 0.0
        self.corner_steer_samples = 0
        self.straight_steer_samples = 0
        self.steer_zero_steps = 0
        self.corner_steer_zero_steps = 0
        self.steer_direction_changes = 0
        self.corner_steer_direction_changes = 0
    ...
    def record(self, S, R, track_info):
        ...
        steer_direction = sign(steer) if abs(steer) > 0.03 else 0
        if steer_direction == 0:
            self.steer_zero_steps += 1
        if self.last_steer_direction != 0 and steer_direction != 0 and steer_direction != self.last_steer_direction:
        ...
        if track_info['is_sharp_corner']:
            ...
            self.straight_abs_steer_sum += abs(steer)
            self.straight_steer_samples += 1
        else:
            self.corner_steps += 1
            self.corner_abs_steer_sum += abs(steer)
            self.corner_steer_samples += 1

        if is_final_corner(S):
            self.final_corner_steps += 1

        if not track_info['is_straight']:
            if steer_direction == 0:
                self.corner_steer_zero_steps += 1
            if self.last_steer_direction != 0 and steer_direction != 0 and steer_direction != self.last_steer_direction:
                self.corner_steer_direction_changes += 1
    ...
    def write_row(self, log_type, reason, lap_time):
        ...
        avg_abs_steer = self.abs_steer_sum / self.steps if self.steps else 0.0
        avg_corner_abs_steer = self.corner_abs_steer_sum / self.corner_steer_samples if self.corner_steer_samples else 0.0
        avg_straight_abs_steer = self.straight_abs_steer_sum / self.straight_steer_samples if self.straight_steer_samples else 0.0
        ...
        row = {
            ...
            'avg_abs_steer': rounded(avg_abs_steer),
            'avg_corner_abs_steer': rounded(avg_corner_abs_steer),
            'avg_straight_abs_steer': rounded(avg_straight_abs_steer),
            'steer_zero_steps': self.steer_zero_steps,
            'corner_steer_zero_steps': self.corner_steer_zero_steps,
            'steer_direction_changes': self.steer_direction_changes,
            'corner_steer_direction_changes': self.corner_steer_direction_changes,
            ...
            'final_corner_steps': self.final_corner_steps,
        ...
        if file_exists:
            with open(LOG_FILE, 'r') as existing_log:
                current_header = existing_log.readline().strip().split(',')
            write_header = current_header != LOG_FIELDS
...
def calculate_target_speed(S, track_info):
    ...
    if is_final_corner(S):
        speed = min(speed, final_corner_target_speed(S))
...
def calculate_throttle(S, R, track_info):
    ...
    if is_final_corner(S):
        final_target = final_corner_target_speed(S)
        if S['speedX'] > final_target - 5:
            accel = min(accel, 0.12)
        elif get_value(S, 'distFromStart') > FINAL_CORNER_EXIT and abs(S['speedY']) < LATERAL_SPEED_SOFT_LIMIT:
...
def apply_brakes(S, track_info):
    ...
    if is_final_corner(S):
        final_target = final_corner_target_speed(S)
        final_overspeed = S['speedX'] - final_target
        if final_overspeed > 30:
            brake = max(brake, MAX_BRAKE)
        elif final_overspeed > 18:
            brake = max(brake, 0.32)
        elif final_overspeed > 8:
    ...
    if abs(S['speedY']) > LATERAL_SPEED_HARD_LIMIT:
        brake = max(brake, 0.25)
    elif abs(S['speedY']) > LATERAL_SPEED_SOFT_LIMIT and is_final_corner(S):
...
def calculate_action_from_sensors(S, previous_action=None):
    R = default_action()
    if previous_action:
        R.update(previous_action)
...
def drive_modular(c):
    S, R = c.S.d, c.R.d
    next_action, track_info = calculate_action_from_sensors(S, R)
    R.update(next_action)
    return track_info

# ================= MAIN LOOP =================
if __name__ == "__main__":
    C = Client(p=3001)
    print("Loaded torcs_jm_par_modulare %s" % DRIVER_VERSION)
